...
test('connection-details route provisions room credentials and starts a bot', async () => {
  const connection = await jsonRequest('/api/connection-details', {
    method: 'POST',
    body: JSON.stringify({}),
  });
...
test('connection-details handles empty and malformed bodies correctly', async () => {
  const emptyBody = await jsonRequest('/api/connection-details', {
    method: 'POST',
  });
  ...
  const malformedBody = await jsonRequest('/api/connection-details', {
    method: 'POST',
