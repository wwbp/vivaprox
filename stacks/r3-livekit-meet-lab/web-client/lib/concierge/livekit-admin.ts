...
export function getRoomServiceClient(): RoomServiceClient {
  ...
  const publicLiveKitUrl = requireEnv(config.livekitUrl, 'LIVEKIT_URL');
  const configuredInternalUrl = process.env.LIVEKIT_URL_INTERNAL;
  const livekitUrl = configuredInternalUrl ?? publicLiveKitUrl;
  const parsed = new URL(livekitUrl);

  // Docker Compose recipe defaults often expose LIVEKIT_URL as localhost for browser clients.
  // Concierge is server-side and should target the container network hostname.
  if (
    !configuredInternalUrl &&
    config.botRunnerUrl?.includes('agent-runner') &&
    (parsed.hostname === 'localhost' || parsed.hostname === '127.0.0.1')
  ) {
    parsed.hostname = 'transport-server';
  }
  ...
  return new RoomServiceClient(
    toHttpLiveKitUrl(parsed.toString()),
    livekitApiKey,
    livekitApiSecret
  );
}
